...
export class RequestService {
    ...
    async getTipo(ruta: string) {
        this.loading = true;
        return new Promise((resolve) => {
            this.http
                .get(
                    `${environment.apiUrl}/${ruta}`).subscribe(
                        (response: any) => {
                            this.loading = false;
                            resolve([true, response]);
                        },
    ...
    async updateTipo(ruta: string, data: any, id: number) {
        this.loading = true;
        return new Promise((resolve) => {
            this.http
                .put(
                    `${environment.apiUrl}/${ruta}/${id}`, data)  .subscribe(
                    (response: any) => {
                        this.loading = false;
                        resolve([true, response]);
                    },
                    (error: any) => {
                        this.loading = false;
                        if (!this.tokenIsValid(error.status)) {
                            this.showAlert("Error al modificar registro", "error");
                        }
                        resolve([false]);
                    }
                );
    ...
    async createTipo(ruta: string, data: any) {
        this.loading = true;
        return new Promise((resolve) => {
            this.http
                .post(
                    `${environment.apiUrl}/${ruta}`, data)  .subscribe(
                    (response: any) => {
                        this.loading = false;
                        resolve([true, response]);
                    },
                    (error: any) => {
                        this.loading = false;
                        if (!this.tokenIsValid(error.status)) {
                            this.showAlert("Error al crear registro", "error");
                        }
                        resolve([false]);
                    }
                );
    ...
    async deleteTipo(ruta: string, id: number) {
        this.loading = true;
        return new Promise((resolve) => {
            this.http
                .delete(
                    `${environment.apiUrl}/${ruta}/${id}`)  .subscribe(
                    (response: any) => {
                        this.loading = false;
                        resolve([true, response]);
                    },
