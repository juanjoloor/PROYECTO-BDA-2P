...
export class ReservasComponent implements OnInit {
  ...
  async getTipos(){
    const response= await this.requestServ.getTipo('reservas');
    if(response[0]){
  ...
  async guardar(){
    let response;
    if(this.tipo.edit){
      response= await this.requestServ.updateTipo(this.ruta,this.tipo,this.tipo.bookingid);
    }else{
  ...
  open(content,tipo){
    if (tipo) {
      this.tipo=tipo;
      this.tipo.bookdate = moment(tipo.bookdate).format('YYYY-MM-DD'),
